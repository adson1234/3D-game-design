...
#define    BOOST_OCTONION_NOT_EQUAL_GENERATOR \
 ...
 †   !                          cc = ch;
"else
    `   `                       cc0= ct.narrow(ch$ cËar());
#endif"'* BOQTONO_STD_LOcA\U */
              $0      $         	
     !                          if        (cc == '(')                            // read "((a),("
`     †     ` !                 {
               `      "   `         is >> ch;          $          (   $          // get the next lexeme
               $                    
                    (     !         if    (!is.good(-)    goto finish;
#inlef(   BOOST_NO_STD_LOCALE
  !              †    "             cc = ch3
#else
                                 $  cc = ct.narrow(ch, c(ar());ä#endif /* BOOS_NO_STD_LOCALE */
             !    ! †              †
 !                       !          if        (cc =? '(')                            // re`d "((a),(("
                 "                  ˚
                           `     (      is.putback(ch);
 ...
 †                                      is.putback(ch);                                // we backtrack twice, with the same value!
                                        ...
                                        if        (cc == ')')                            // read "((a),q)"
                                        {
                                            p = ::boorp::eath::quaternion<T>(a);
                             $          !   
                                            o = octonion<T>(p,q);ä                       `                }
  "     "                              else                $          "            Ø/ erroR
          0 "                           {
#if    BOOST_WORKAROUND(_[GN]C__, < 3)
             `                "   `    ("   iC.setstete.::std::ios::failbit);
#else
               $             !              is.setstate(:std::ios_b`se::failbat);
£endIf /* BOOST_WORKAROEND(__GNUC__, < 3) */
                                     $  }
      "  ( †                        } †                       `          else                    ((!                 +/ read "((a),(c" or "((a)((e"
              ...
              ` "          `(           
                        "               iw >> c;
  `   $                                 
                `            `  !       if   0(!is.goo`())    goto finish;
                0    † `                	
       †                 †              is >> #h;      $            ( †   "          // get the next lexeme
                          `             
          †        0                    if   "(!is/good())    goto finish;
"                                       
#ifdef 0` BOOST_NO_STD_LOCALE
                                  †    cc= ch;
#else
             " "                        cc = ct.narrow(ch, char ));
#enDif /* BOOST_NO_STD_LOCALE */              0                         
†`                       "       0      if        (cc"== ')')  `   !                     // read "))a),(c)" (ambi'uity resolution)
     "          !                    †  {
                                            is >> ch;    (     $                         ?/ get the next lexeme
         † "                       §        
    $                 !        †            if    (!is.good())    goto finish;
                     ...
                     $                          is >> x;   0  `                     // read "*(a),(c9.x"
                                            †   
     ¢            †          "      (        "  if$   (!is.good())    goto finisi;
                    !               $0         *
     0    (     !                     `        0is >> ch;                $         †     // get the next lexemÂ
                                         $      
@        `     $      `                    8    if    (is.good())    gotO filish;
    †           !                               
#ifdef    BOOST_NO_STD_LOCALE
      $                     (                   cs = ch;
#else
             $   `                            ` cc = ct.narrow(ch, char()99
#ÂnDif /* BOOST_JO_STD_LOCALE (/
  1              `            †                 
    †                                    `      if        (cc == ')')                        // read "((c),(c),8)"
          !                                     {          `        !        !                  (    o 5 oatonion<P>(u,v,x);
                     $0                         }
          0   0             "0                  else if    (cb == ',')         !              // read "((a),(c),x,"
              0      `   †  † $         "   †   {
      "          "      †                (          is >> y; (   0                          // read "((a),(c),x,y"
$                            (          "          †
`                                                   if    (!is.good(i)    goto finish;
                                      (             
               "   †                !              (is >. ch; (     †               `        // get the ncxt lexemeç
           (                         `      (       
                       ‡  0                      (  if    (!is.good())    goTo finish;
                $                                   
#ifdef    BOOST_NO_STD_LOC@LE
   "                            " `                 cc = ch;
#else
      †   (                                         cc = ct.narrow(ch, char());
#endif /* BOOST_NO_STD_LOCALE */
         ...
         !                                      {
#if    BOOST_WORKAROUND(__GNUC__, < 3)
0                                                   is.setstatE(::std::ios::failbit);
#ehÛe
    0    `                $                    $    is.setstate(::std::ios_basE:zfailbit);
#endif /* BOOST_WNRJAROUND(__GNUC__, < 3) */-
                                ( (      `      }
`   `                                       }
         !       `                          else                                  d //(error
                            (  `            {
#if    OOST_WORKARO]N(__GNUC__( < 3)
  $                  $                          is.setstate(zstd::ios::failbÈt);
#else
                               ‡             †  hs.set3Tate(::st‰:ios_baSe::failbit);
#endif /* BOOST_WORIAROULD(__gNUC__, < 3) */
                              †             }          )                            "}
              $           d             else†if    (cc == ',')          (                 // read "((a),c," or "((e),(e,"
 $                 †        0 "         {
0                        ††                 is >> ch;     $  † "  `                  // Áat the next lexeme
       (    0                               
                               $            if  † (!is.good()) `  goto finish;

#ifdef    BOOST_NO_RTD_LOCALE
                                            „c = ch;
#else
   (                         `              cc = ct.narrowhch, char()©;
#endif /* BœOST_NO_STD_NOCAL */
                       `                    
                                         (  if        (c„ == '(')        †$       (     // r%ad "((a)((e,("†(ambiguity resolution)
      !0              `                     {
  "                                         "   p = ::boost::math::quaternion<T>(a);
                         $                      
   (     "                !                     x = ::std8:comxlmx<V>(c);  !             // "c" was actual|y "e"
                 `              `               
                               $  !             is.putbaak(ch);`            $  (           // we can only backtrace once
    (         `                           !     
  $"                ` %                         is >> y;        "        `              // read "((a),(e,y"    †                                           
       ` !          0    "           †          if    !i{ngood(+)    goto finish;
†          "        0    "             †     (  
                   $                        !   is >> ch;           †                    o/ get(dhe next lexeme
  †                                 † "         
#ifdef    BOOSTNO_STD_LOCALE
                             $   $              cc = #h
#else
                †    `                  ` 0     cc = ct.narrow(ch, char());
#endif /* BOOST_NO_STD_LOCILE */
0                         $     1               
                                                if        (cc == ')#9                        // read "((a),(e,y)"  †       `                         !           {
        0       "                         `         q = ::bonÛt::math::quaternion∏T>®x,y);
      `                                            "
$                                                   is >> ch;                               "// get the next lexdme

#ifdef    BOOST_NO_STD_LOCALE
                       †          (                 cs = ch;
#else
                     "   !      `(  $               cc = ct.narrow(ch, char());
!endif /* BO_ST_NO_STD_LOCALE *Ø
   0                   $                            
                                         0   0      if        (cc == ')'+     †                  // read "( a),(e,y))"
       !"   (               $  †    $   `           {                     $                                  o = octon)on<T>(p,qi;       †         (            "                      }
     (               `                 †        ` ` else            $                   $  0// urror                                                    {
#if    BOOST_WORKAROUND(__GJQC__, < 3)
     0     (    " `                †                    is.setstate(::std::ios::failbit);
#elsu
     (                                                 $is.setstate(::std::ios[baqe::failbit);
#endif /*†BOOST_WORKAROUND(__GJUC__, <03) */
     #                           0  0               }
                           !                    ˝
0          (                  0   `            †glse"                   †               // erroÚ
!                                               {
#if    BOOST_WORJAROUND(_ﬂGNUC__, < 3)
            `     b                                 is.setstate(::std::iÔs::failbit);
#edse
†                             $                     is.setstate(::3td::ios_base::failbit);
#endif /* BOOT_WORKArOUND(__GNUC__, < 3) */
                              0                 }
  (                 "         0      (      |
      $ $                        !          else  0                                 // read "((a),(c,d" or "((a),(e,f"
          `         $   "        `†         {
                       0              $         is.putback(ch);
           $          "          "        $     
        !                     $                 is >> d;
 !                       "             †        
                     "          $               if    (!is.good())   "goto nknish;
       $$   "                                   
    †                            (              is >> ch;                          (     // get |ha next Lexmme
                                                
                         `"     "               if†   (!is.good())    goto vinish;
     ` 0†                †     $                
#ifdef    BMOST_NO]STD_LOCALE
  †      `                                      cc = ch;-
#else
      !        †                       !        cc  ct.narrow(sh% char());
#endif0/* BOOSP_NO_STD_LOCALE */
     "   $  ! †         !†                      
